...
def create_book_in_db(user_id: str, id: str):
    book_doc_ref = db.collection('users').document(user_id).collection('books').create({
        'id': id,
...
def create_page_in_db(user_id: str, book_id: str, id: str): 
    page_doc_ref = db.collection('users').document(user_id).collection('books').document(book_id).collection('pages').create({
        'id': id,
        'creationDate': datetime.now,
        'imageOptions': [],
        'status': 'pending'
